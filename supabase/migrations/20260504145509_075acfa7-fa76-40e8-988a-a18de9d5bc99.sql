-- 1. Atualizar a função de finalização de proposta
CREATE OR REPLACE FUNCTION public.finalize_public_proposal(_public_token uuid, _payload jsonb DEFAULT '{}'::jsonb)
 ...
 BEGIN
   ...
   -- Busca o nome do cliente se não estiver no payload
   IF v_client_name IS NULL OR v_client_name = '' THEN
     SELECT name INTO v_client_name
     FROM public.proposal_parties
     WHERE proposal_link_id = v_link.id AND role = 'primary_tenant'
     LIMIT 1;
   END IF;
 
   -- Formata o título: {Cliente} — {Imóvel}
   v_title := CASE 
     WHEN v_client_name IS NOT NULL AND v_client_name <> '' AND v_property_info IS NOT NULL AND v_property_info <> '' 
       THEN v_client_name || ' — ' || v_property_info
     WHEN v_client_name IS NOT NULL AND v_client_name <> '' 
       THEN v_client_name
   ...
   ELSE
     -- Atualizar card existente (inclusive o título)
...
-- 2. Corrigir títulos de cards existentes no board de Locação
-- Board ID: 3b619b46-85bf-487d-955b-e1255b1bf174
WITH primary_tenants AS (
  SELECT card_id, name 
  FROM public.proposal_parties 
  WHERE role = 'primary_tenant' AND card_id IS NOT NULL
)
UPDATE public.cards c
SET title = pt.name || ' — ' || COALESCE(c.building_name, c.robust_code, '')
FROM primary_tenants pt
WHERE c.id = pt.card_id
AND c.board_id = '3b619b46-85bf-487d-955b-e1255b1bf174'
AND (c.title IS NULL OR c.title NOT LIKE pt.name || ' — %');
