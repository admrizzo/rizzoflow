-- Aplica automaticamente os templates de checklist marcados como ativos
-- (board_config.auto_apply_checklist_templates) ao criar um card via
-- finalize_public_proposal. Evita duplicidade verificando o nome.

CREATE OR REPLACE FUNCTION public.apply_active_checklist_templates(_card_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  SELECT board_id INTO v_board_id FROM public.cards WHERE id = _card_id;
  IF v_board_id IS NULL THEN RETURN; END IF;

  SELECT COALESCE(auto_apply_checklist_templates, ARRAY[]::uuid[])
    INTO v_template_ids
  FROM public.board_config WHERE board_id = v_board_id;
  ...
  FOR v_tpl IN
    ...
    ORDER BY position
  LOOP
    -- Anti-duplicidade por nome
    IF EXISTS (
      SELECT 1 FROM public.checklists
      WHERE card_id = _card_id AND name = v_tpl.name
    ) THEN
      CONTINUE;
    END IF;
    ...
    FROM public.checklists WHERE card_id = _card_id;

    INSERT INTO public.checklists (card_id, name, position)
    VALUES (_card_id, v_tpl.name, v_next_position)
    RETURNING id INTO v_new_checklist_id;

    INSERT INTO public.checklist_items
      (checklist_id, content, position,
       requires_date, requires_status, requires_observation, status_options)
    SELECT v_new_checklist_id, cit.content, cit.position,
           COALESCE(cit.requires_date, false),
           COALESCE(cit.requires_status, false),
           COALESCE(cit.requires_observation, false),
           COALESCE(cit.status_options, '[]'::jsonb)
    FROM public.checklist_item_templates cit
    WHERE cit.template_id = v_tpl.id
    ORDER BY cit.position;
  END LOOP;
END;
$$;

-- Atualizar a RPC de finalização para aplicar templates ativos
...
BEGIN
  ...
  -- Aplica templates ativos do board (idempotente: pula por nome se já existir)
...
-- Permitir que NewProposalButton (ou qualquer fluxo authenticated) também
-- aplique os templates ao gerar o card inicial:
GRANT EXECUTE ON FUNCTION public.apply_active_checklist_templates(uuid) TO authenticated;