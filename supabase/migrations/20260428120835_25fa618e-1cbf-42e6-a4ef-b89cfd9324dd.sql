-- Convert cards.next_action_due_date from DATE to TIMESTAMPTZ
ALTER TABLE public.cards
  ALTER COLUMN next_action_due_date TYPE timestamp with time zone
  USING (next_action_due_date::timestamp AT TIME ZONE 'America/Sao_Paulo');

-- Drop and recreate get_my_queue (return type changed)
...
AS $function$
  WITH visible_cards AS (
    SELECT
      ...
      c.created_by, c.column_entered_at
    FROM public.cards c
