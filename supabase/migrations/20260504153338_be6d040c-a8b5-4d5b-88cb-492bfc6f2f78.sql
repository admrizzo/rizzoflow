CREATE OR REPLACE FUNCTION public.finalize_public_proposal(_public_token uuid, _payload jsonb DEFAULT '{}'::jsonb)
 ...
 BEGIN
   ...
   -- Busca o nome do cliente se não estiver no payload (priorizando locatário principal)
   ...
   FROM public.boards
   WHERE is_active = true AND (lower(name) LIKE '%loca%' OR name = 'Fluxo de Locação')
   ORDER BY position
   LIMIT 1;
   ...
   ELSE
     ...
     UPDATE public.cards SET
       ...
       service_broker_id = COALESCE(service_broker_id, v_link.broker_user_id),
       updated_at = now()
     WHERE id = v_card_id;
   END IF;
