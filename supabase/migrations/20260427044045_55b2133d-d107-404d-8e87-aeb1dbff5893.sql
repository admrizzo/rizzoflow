-- RPC otimizada para a tela "Minha Fila".
-- Roda como SECURITY INVOKER -> as RLS de `cards` se aplicam ao usuário chamador,
-- então o escopo (admin / gestor / administrativo / corretor) é respeitado naturalmente.
...
RETURNS TABLE (
  ...
  checklist_open_doc_items bigint,
  is_overdue boolean,
...
AS $$
    ...
    SELECT
      ...
      COUNT(*) FILTER (
        WHERE COALESCE(ci.is_dismissed, false) = false
          ...
          AND (
            lower(ci.content) LIKE '%document%'
  ...
  SELECT
    ...
    vc.next_action,
    vc.next_action_due_date,
    vc.responsible_user_id,
    p.full_name AS responsible_name,
    vc.created_by,
    vc.column_entered_at,
    ...
    (vc.next_action_due_date IS NOT NULL AND vc.next_action_due_date < CURRENT_DATE) AS is_overdue,
    (vc.next_action_due_date IS NOT NULL AND vc.next_action_due_date = CURRENT_DATE) AS is_due_today,
  ...
  ORDER BY
    CASE
      WHEN vc.next_action_due_date IS NOT NULL AND vc.next_action_due_date < CURRENT_DATE THEN 0
      WHEN vc.next_action_due_date IS NOT NULL AND vc.next_action_due_date = CURRENT_DATE THEN 1
...
GRANT EXECUTE ON FUNCTION public.get_my_queue() TO authenticated;