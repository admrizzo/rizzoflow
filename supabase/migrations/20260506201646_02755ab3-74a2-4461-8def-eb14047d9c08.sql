...
AS $function$
  ...
  BEGIN
    ...
    IF v_correction_request_id IS NOT NULL THEN
      ...
      ELSE
        -- Se foi passado um ID mas não pôde ser atualizado (não pendente ou não do link)
        IF NULLIF(_payload->>'correction_request_id', '') IS NOT NULL THEN
          RAISE EXCEPTION 'Correção pendente não encontrada ou já respondida para esta proposta';
        END IF;
      END IF;
    END IF;
